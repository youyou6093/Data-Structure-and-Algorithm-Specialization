...
class graph:
    def __init__(self,vertexs):
        self.vertexs=[[] for i in vertexs]
        
    def add_edge(self,edge):
        self.vertexs[trans(edge.from_)].append(trans(edge.to_))  #add a new edge
...
def main():                
    ...
    for i in clauses:
        original_graph.add_edge(edge(-i[0],i[1]))
        original_graph.add_edge(edge(-i[1],i[0]))
        inverse_graph.add_edge(edge(i[1],-i[0]))
        inverse_graph.add_edge(edge(i[0],-i[1]))
    inverse_graph.DFS()
    ...
    all_scc=[]              #get all scc componentes(this one already have order)
    for i in post_order:        #make sure that only one of two values appear in an scc
    ...
    if valid_or_not(all_scc):
        ...
        for i in values:
            if values[i]==1:
                order.append(i)               #get values of all vertex based on scc
